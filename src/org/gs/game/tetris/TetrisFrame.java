...
public class TetrisFrame extends JFrame implements ActionListener, KeyListener
{
    ...
    private static final long[] tetrisSpeeds
    ...
    // 레벨업이 몇 삭제행 단위로 동작하는지 (5이상으로 설정)
    private static final int levelupRows  = 25;
    private static enum GameStatus { STOPPED, STARTED, PAUSED };
    ...
    public void actionPerformed(ActionEvent e)
    {
        if (e.getSource() instanceof JButton
            && (TetrisBoard.START_LABEL.equals(((JButton)e.getSource()).getText())
        ...
        else if (e.getSource() instanceof JMenuItem)
        {
            ...
            if (MENU_STOP_GAME.equals(actionCmd))
                ...
                showRank(false);
            else if (TetrisBoard.START_LABEL.equals(actionCmd)
                     || TetrisBoard.PAUSE_LABEL.equals(actionCmd))
                controlGame();
            else
            {
                TetrisMode newMode = Enum.valueOf(TetrisMode.class, actionCmd);
                if (gameMode != newMode)
                {
                    setVisible(false);
                    setGameMode(gameMode, newMode);
                    pack();
                    Tetris.moveToCenter(this);
                    setVisible(true);
                }
            }
        }
    ...
    private void initContentPane()
    {
        ...
        gameGrids[0] = new TetrisGrid(12, 28, false, false);
        ...
        setGameMode(null, gameConfig.getTetrisMode());
    }
    
    private void setGameMode(TetrisMode prevMode, TetrisMode newMode)
    {
        if ((prevMode == null || prevMode == TetrisMode.SINGLE)
             && newMode != TetrisMode.SINGLE)
        {
        ...
        {
            gameGrids[1] = new TetrisGrid(12, 28, false, false);
        ...
        if (newMode == TetrisMode.COOP)
            gameGrids[0].setSize(18, 28, true);
        else if (prevMode == TetrisMode.COOP)
            gameGrids[0].setSize(12, 28, false);
     ...
     * Called by timer
     * 
     * @param nActive the active user index
     */
    ...
    private void moveShape(int x, int y, int nActive)
    {
        ...
        if (y < 0)
        {
            ...
            {
                TetrisGrid.CanMove canMove;
                while ((canMove = gameGrid.canMoveShape(x, y, nActive))
                        == TetrisGrid.CanMove.CAN)
                    y++;
    ...
    private void rotateShape(int nActive)
    {
        ...
        synchronized (this)
        {
            if (gameGrid.canMoveShape(0, 0, rotatedShape.getPoints(), nActive)
                == TetrisGrid.CanMove.CAN)
            {
    ...
    private boolean newShape(int nActive)
    {
        ...
        if (getGameGrid(nActive).setBlockShape(tetrisBlocks[nActive].getBlockShape(false),
                                               nActive, true))
            setTimeout(nActive);
        else
            bStopGame = true;
    ...
    private void addComponent(Component component, Container boxOuter,
                              GridBagLayout gbl, GridBagConstraints gbc, boolean bInset)
    {
    ...
    private void showPopupMenu(MouseEvent e)
    {
        ...
        String startPause = gameStatus == GameStatus.STARTED
                            ? TetrisBoard.PAUSE_LABEL : TetrisBoard.START_LABEL; 
        ...
        else
        {
            ...
            for (TetrisMode mode: TetrisMode.values())
            {
                JRadioButtonMenuItem cbmi = new JRadioButtonMenuItem(mode.name());
                
                cbmi.setActionCommand(cbmi.getText());
                if (gameMode == mode)
